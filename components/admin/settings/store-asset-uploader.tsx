...
import { deleteStoreAsset, uploadStoreAsset } from "@/lib/admin/store-assets";
...
// Single-image uploader for the store logo/favicon — same shape as CategoryImageUploader, but
// against the store-assets bucket.
export function StoreAssetUploader({ image, onChange }: StoreAssetUploaderProps) {
  ...
  async function handleFiles(files: FileList | null) {
    ...
    onChange(url);
    if (previous) void deleteStoreAsset(previous);
  }

  function removeImage() {
    if (image) void deleteStoreAsset(image);
    onChange(null);
  }
